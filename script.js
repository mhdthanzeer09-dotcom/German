...
const chapterId = document.body.dataset.chapter; // e.g., "chapter1", "chapter2"
let learnedWords = JSON.parse(localStorage.getItem(chapterId)) || [];
updateProgress();
...
words.forEach(word => {
    word.addEventListener("click", () => {
        ...
        if(!learnedWords.includes(word.textContent)){
            learnedWords.push(word.textContent);
            localStorage.setItem(chapterId, JSON.stringify(learnedWords));
            updateProgress();
        }
    });
});
